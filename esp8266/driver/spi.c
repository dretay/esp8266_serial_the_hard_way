...
void spi_mast_set_mosi(uint8 spi_no, uint8 offset, uint8 bitlen, uint32 data)
{
    ...
    os_printf("this is the first line\n\r");
...
void spi_byte_write_espslave(uint8 spi_no,uint8 data){
  ...
  //SPI_USR_COMMAND_BITLEN 0x0000000F
...
void spi_slave_isr_handler(void *para)
{
  ...
  if(READ_PERI_REG(0x3ff00020)&BIT4){
    ...
    if(regvalue&SPI_SLV_WR_BUF_DONE){
      ...
      system_os_post(0, 0x00, (os_param_t)&spi_data);

      //GPIO_OUTPUT_SET(0, 1);

    }
    if(regvalue&SPI_SLV_RD_BUF_DONE){
      system_os_post(0, 0x01, (os_param_t)&spi_data);
